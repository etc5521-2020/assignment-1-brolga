...
### analysis the species of villagers
```{r analysis species}
dat <- villagers%>%
  select(species)%>%
  group_by(species)%>%
...
dat4 <- dat3%>%
 ...
 knitr::kable(head(dat4,10),
  caption = "villagers birthday month analysis") %>% 
  kable_styling(latex_options = c("striped", "hold_position"))
